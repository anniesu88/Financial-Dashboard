...
儀表板不用改任何程式碼就能部署到
[Streamlit Community Cloud](https://share.streamlit.io/)。連結這個 repo 後設定：

| 設定項目 | 值 |
|---|---|
| 主檔案路徑 | `dashboard_20260805.py` |
| Python 版本 | 3.11 |

再到 app 的 **Settings → Secrets** 加入兩個設定：

```toml
SEC_CONTACT_EMAIL = "you@example.com"
ADMIN_PASSWORD    = "設一個夠長的密碼"
```

### 管理者密碼閘門

有三個功能會寫入檔案——**新增公司**、**刷新 EDGAR 資料**、
**自訂比率產生器**。部署到雲端後所有訪客共用同一個檔案系統，而且這些操作
發出的 SEC 請求會帶著你的聯絡信箱，所以它們被密碼保護：

- **沒有設定 `ADMIN_PASSWORD`**（本機執行的情況）→ 功能全開，行為與你在
  自己電腦上執行時完全相同。
- **有設定 `ADMIN_PASSWORD`**（雲端）→ 訪客看到的是唯讀儀表板：所有圖表、
  比率、公式說明、原始財報都完整可見。在側邊欄輸入密碼即可解鎖那三個功能。

部署前要知道的兩件事：

- **變更是暫存的**：容器重啟後檔案系統會還原，所以透過網站新增的公司，在
  app 休眠或重新部署後就會消失。要永久保留請在本機執行 fetch 後 push 更新
  後的 JSON。
- **從私有 repo 部署**需要額外授權 Streamlit 存取你的 GitHub 帳號，且免費
  方案對私有 app 有額度限制，請自行確認當前條款。另外要注意：**即使 repo
  是私有的，app 網址本身仍是公開的**，除非你在 app 設定中限制檢視者。
